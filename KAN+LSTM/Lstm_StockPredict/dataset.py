...
def getData(corpusFile,sequence_length,batchSize):
    ...
    stock_data.drop('pre_close', axis=1, inplace=True)  # 删除列pre_close 前一天的收盘价列
    ...
    for i in range(df.shape[0] - sequence):
        X.append(np.array(df.iloc[i:(i + sequence), ].values, dtype=np.float32))  # 构造特征  取从第 i 天开始的sequence天的数据
        Y.append(np.array(df.iloc[(i + sequence), 0], dtype=np.float32)) # 构造标签  取第 i + sequence 天的收盘价
    ...
    train_loader = DataLoader(dataset=Mydataset(trainx, trainy, transform=transforms.ToTensor()), batch_size=batchSize,
                              shuffle=True)
    test_loader = DataLoader(dataset=Mydataset(testx, testy), batch_size=batchSize, shuffle=True) # 测试集数据加载器
    return close_max,close_min,train_loader,test_loader  # 返回最大值、最小值、训练集和测试集数据加载器
